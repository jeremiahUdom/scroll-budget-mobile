import { getTodaysDateString } from "../getTodaysDate";
import { getData, saveData } from "../storage";
import {
  SCROLL_BUDGET_KEY,
  SCROLL_BUDGET_LAST_UPDATED_AT_KEY,
} from "./localStorage";
...
export const setScrollBudget = async (budget: number) => {
  ...
  await saveData(SCROLL_BUDGET_LAST_UPDATED_AT_KEY, getTodaysDateString());
};

export const getScrollBudgetLastUpdatedAt = async () => {
  const scrollBudgetLastUpdatedAt = await getData<string>(
    SCROLL_BUDGET_LAST_UPDATED_AT_KEY,
  );
